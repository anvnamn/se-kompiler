...
void compare_tokenstreams(TokenStream &actual_ts, TokenStream &expected_ts)
{
    ...
    ASSERT_EQ(actual_ts.size(), expected_ts.size()) << fmt::format("Mismatch in number of tokens, actual {}, expected {}", actual_ts.size(), expected_ts.size());
    ...
    while (actual_ts.peek())
    {
        ...
        EXPECT_EQ(to_string(*actual_token), to_string(*expected_token))
            << fmt::format("Mismatch, actual {}, expected {}", to_string(*actual_token), to_string(*expected_token));
    }
}
...
TEST(LexerTest, BasicProgram)
{
    ...
    expected_tokens.emplace_back(std::make_unique<DataTypeToken>(Datatype::INTEGER));
    ...
    expected_tokens.emplace_back(std::make_unique<DataTypeToken>(Datatype::INTEGER));
    expected_tokens.emplace_back(std::make_unique<IdentifierToken>("foersta_talet"));
